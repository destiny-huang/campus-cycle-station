...
function contentText(body: Record<string, any>) {
  ...
  if (Array.isArray(content)) return content.map((item) => typeof item?.text === 'string' ? item.text : typeof item?.content === 'string' ? item.content : '').join('');
  if (content && typeof content === 'object' && typeof content.text === 'string') return content.text;
  throw new AiError('invalid_response', 'AI 未返回有效内容');
}

function decodeVisionObject(text: string) {
  let candidate: unknown = text.trim().replace(/<think>[\s\S]*?<\/think>/gi, '').trim()
  ...
  for (let depth = 0; depth < 3; depth += 1) {
    ...
    try { candidate = JSON.parse(serialized); }
      ...
      const start = serialized.indexOf('{'); const end = serialized.lastIndexOf('}');
      if (start < 0 || end <= start) throw new AiError('invalid_response', 'AI 识别结果格式无效');
      try { candidate = JSON.parse(serialized.slice(start, end + 1)); }
      catch { throw new AiError('invalid_response', 'AI 识别结果格式无效'); }
    }
  }
  if (Array.isArray(candidate) && candidate.length === 1) candidate = candidate[0];
  if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) throw new AiError('invalid_response', 'AI 识别结果格式无效');
  let object = candidate as Record<string, unknown>;
  for (const key of ['donation_vision', 'vision_result', 'result', 'data']) {
    const nested = object[key];
    if (nested && typeof nested === 'object' && !Array.isArray(nested)) { object = nested as Record<string, unknown>; break; }
  }
  return object;
}

export function parseVisionResult(text: string, fallbackCategory = 'book'): VisionResult {
  const raw = decodeVisionObject(text);
  const safeFallback = allowedCategories.includes(fallbackCategory as typeof allowedCategories[number]) ? fallbackCategory : 'book';
  const rawCategory = raw.category ?? raw.category_id;
  const rawCondition = raw.condition ?? raw.condition_key;
  const category = allowedCategories.includes(rawCategory as typeof allowedCategories[number]) ? String(rawCategory) : safeFallback;
  const condition = Object.hasOwn(conditionMap, String(rawCondition)) ? String(rawCondition) as keyof typeof conditionMap : 'good';
  const nameValue = raw.object_name ?? raw.objectName ?? raw.name;
  const objectName = typeof nameValue === 'string' && nameValue.trim() ? nameValue.trim().slice(0, 100) : '照片中的物品';
  ...
  const confidenceValue = typeof raw.confidence === 'number' ? raw.confidence : typeof raw.confidence === 'string' ? Number(raw.confidence) : 0;
  ...
  return { objectName, category, condition, visibleIssues: stringList(raw.visible_issues ?? raw.visibleIssues), manualChecks: stringList(raw.manual_checks ?? raw.manualChecks), confidence,
    basePoints: categoryConfig.base_points, multiplier, suggestedPoints: Math.floor(categoryConfig.base_points * multiplier + 0.5) };
}

export function parseVisionResponse(body: Record<string, any>, fallbackCategory = 'book') {
  return parseVisionResult(contentText(body), fallbackCategory);
}
  ...
  try {
    ...
    const response = await client.chat('vision', client.config.visionModel, {
      ...
      response_format: { type: 'json_schema', json_schema: { name: 'donation_vision', strict: true, schema: { type: 'object', additionalProperties: false,
        ...
        required: ['object_name', 'category', 'condition', 'visible_issues', 'manual_checks', 'confidence'] } } }, reasoning: { effort: 'none' }, max_tokens: 500, temperature: 0,
    });
    aiResponse = response;
    const result = parseVisionResponse(response, donation.categoryId);

...
const wrapped = parseVisionResult('```json\n' + JSON.stringify({ result: { objectName: '兼容图书', category_id: 'book', condition_key: 'good', visibleIssues: ['封面磨损'], manualChecks: ['是否缺页'], confidence: '0.88' } }) + '\n```');
assert(wrapped.objectName === '兼容图书' && wrapped.category === 'book' && wrapped.condition === 'good' && wrapped.confidence === 0.88 && wrapped.manualChecks.length === 1, 'provider wrappers and compatible field shapes must normalize safely');
const doubleEncoded = parseVisionResult(JSON.stringify(JSON.stringify({ object_name: '双重编码', category: 'book', condition: 'good', visible_issues: [], manual_checks: [], confidence: 0.7 })));
assert(doubleEncoded.objectName === '双重编码' && doubleEncoded.confidence === 0.7, 'double-encoded structured JSON must normalize safely');
